...
class Miscellaneous(BaseCog):
    ...
    @bridge.bridge_command(name="avatar", aliases=["av"], description="Gives the profile picture.",
                           description_localizations={"fr": "Donne l'image de profil."})
    @discord.option(name="user", name_localizations={"fr": "utilisateur"},
                    input_type=discord.User, required=False,
                    description="The user whose profile picture you want.",
                    description_localizations={"fr": "L'utilisateur dont vous souhaitez l'image de profil."})
    ...
    async def get_avatar(self, ctx: bridge.BridgeContext, user: discord.User = None):
        ...
        embed.set_footer(text=lang.DEFAULT_FOOTER.format(
            author=ctx.author), icon_url=ctx.author.avatar)
        embed.set_author(name=user)
        embed.set_image(url=user.avatar)
        await ctx.respond(embed=embed)

    @bridge.bridge_command(name="userinfo", aliases=["uinfo", "user-info"],
                           description="Gives info about an account.",
    ...
    @discord.option(name="user", name_localizations={"fr": "utilisateur"}, input_type=discord.User,
                    description="The user you want more info about.",
                    description_localizations={"fr": "L'utilisateur sur lequel voulez plus d'infos."})
    ...
    async def get_user_info(self, ctx: bridge.BridgeContext, user: discord.User = None):
        ...
        async with ctx.typing():
            ...
            embed = discord.Embed(title=lang.GET_USER_INFO_TITLE,
                                  color=user.accent_color or discord.Color.dark_gold())
            if user.avatar:
                embed.set_thumbnail(url=user.avatar)
            embed.add_field(name=lang.GET_USER_INFO_FIELD1_NAME,
                            value=lang.GET_USER_INFO_FIELD1_VALUE.format(
                                user=user, user_id=user.id,
                                date_creation_date=date_timestamp(
                                    user.created_at),
            ...
            if potential_member:
                status = {"online": "🟢", "dnd": "⛔",
                          "idle": "🌙", "offline": "🔌"}
                embed.add_field(name=lang.GET_USER_INFO_FIELD2_NAME,
                                value=lang.GET_USER_INFO_FIELD2_VALUE.format(
                                    nickname=user.display_name,
                                    joined_at=date_timestamp(
                                        user.joined_at),
                                    relative_joined_at=relative_timestamp(
                                        user.joined_at),
                                    activity=user.activity.name 
                                            if user.activity else 'N/A',
                                    status=status[str(user.status)],
    ...
    @bridge.bridge_command(name="serverinfo", aliases=["sinfo", "server-info"],
                           description="Gives info about the server you're in.",
    ...
    async def get_server_info(self, ctx: bridge.BridgeContext):
        ...
        async with ctx.typing():
            embed = discord.Embed(title=lang.GET_SERVER_INFO_TITLE,
                                  description=lang.GET_SERVER_INFO_DESCRIPTION.format(
                guild=guild.name), color=discord.Color.dark_gold())
            if guild.icon:
                embed.set_thumbnail(url=guild.icon if guild.icon else None)

            embed.add_field(name=lang.GET_SERVER_INFO_FIELD1_NAME,
                            value=lang.GET_SERVER_INFO_FIELD1_VALUE.format(
            ...
            embed.add_field(name=lang.GET_SERVER_INFO_FIELD2_NAME,
                            value=lang.GET_SERVER_INFO_FIELD2_VALUE.format(
            ...
            embed.add_field(name=lang.GET_SERVER_INFO_FIELD3_NAME,
            ...
            embed.add_field(name=lang.GET_SERVER_INFO_FIELD4_NAME,
                            value=emojis_str 
                                if emojis_str != "" else lang.GET_SERVER_INFO_FIELD4_VALUE)

        await ctx.respond(embed=embed)

    @bridge.bridge_command(name="id", description="Get the id of an account.",
                           description_localizations={"fr": "Obtiens l'id d'un compte."})
    @discord.option(name="user", name_localizations={"fr": "utilisateur"}, input_type=discord.User)
