...
__all__ = ("PrefixedNotAvailable", "PluginDisabledError", "MissingArgumentsError",
           ...
           "DeprecatedBotError", "PluginWithSameNameError")
...
class MissingArgumentsError(commands.CheckFailure):
    ...
    def __init__(self, command: commands.Command, error_case_msg: str | None = None,
...
class PluginWithSameNameError(Exception):
    ...
    def __init__(self, plugin=None, confilcted_plugin=None, message: str | None = None):
        if not message:
            ...
            conf_plugin_name = ("another plugin" if not confilcted_plugin
                                else f"'{confilcted_plugin.__name__}'")
            name = f" : '{plugin.plugin_name}'" if plugin and confilcted_plugin else ""
            message = (
                f"{plugin_name} has the plugin name for the database as {conf_plugin_name}{name}." + \
                " Please change the name or contact the developper of the extension to correct this conflict."
                )
        super().__init__(message)
